...
Before making changes, read and follow the universal safety rules:
...
Abyss is a drag-and-drop metadata stripping app for photos and videos.
Be careful with media processing:
- Preserve original user files unless the task explicitly says to process in
  place.
- Do not change metadata stripping behavior, output formats, or overwrite
  behavior unless requested.
- Prefer clear warnings for destructive media operations.

---

## Test Coverage

`tests/` does not yet exist for this app. This is a pre-existing gap — no
tests were removed or broken. When adding tests, create `tests/` and follow
the pytest conventions in `pyproject.toml` (`pythonpath = ["src"]`).
